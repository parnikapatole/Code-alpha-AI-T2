...
# This allows Vercel to see a 'handler' to prevent the 500 error
...
# Path handling for Vercel deployment
base_path = os.path.dirname(__file__)
json_path = os.path.join(base_path, 'faqs.json')

if not os.path.exists(json_path):
    st.error(f"❌ ERROR: 'faqs.json' file not found at {json_path}")
else:
    try:
        ...
        # --- 4. NLP ENGINE ---
        # Cache this so the 500 error doesn't happen due to slow processing
        @st.cache_resource
        def load_nlp_model(questions):
